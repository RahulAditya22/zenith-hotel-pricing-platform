...
async def list_properties(db: AsyncSession = Depends(get_db)):
    stmt = select(Property).order_by(Property.star_rating.desc())
...
@router.get("/{property_id}/rooms", response_model=List[RoomTypeResponse])
async def list_property_rooms(property_id: int, db: AsyncSession = Depends(get_db)):
    stmt = (
        select(Property)
