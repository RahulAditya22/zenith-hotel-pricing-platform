"""Concurrency-safe booking service with dynamic pricing."""
...
# Process-level lock for concurrency safety.
# NOTE: this lock is per-process. In a multi-worker deployment
# (uvicorn --workers N), database-level locking or a
# distributed lock (Redis/Redlock) is needed instead.
...
class BookingService:
    ...
    async def get_booked_count_per_date(
        ...
        """Count confirmed bookings per date in [start, end)."""
    ...
    async def calculate_stay_pricing(
        ...
        city = (
            room_type.property.city
            if room_type.property
            else None
        )
        ...
        while curr_date < check_out_date:
            ...
            date_event = next(
                (
                    e for e in events
                    if e.start_date <= curr_date <= e.end_date
                ),
                None,
            )
            event_mult = (
                date_event.multiplier if date_event else 1.0
            )
            event_name = (
    ...
    async def create_reservation(
        ...
        async with _booking_lock:
            # Eagerly load property for pricing queries
            room_stmt = (
                select(RoomType)
            ...
            if not room_type:
                raise HTTPException(
                    ...
                    detail=(
                        f"RoomType ID {payload.room_type_id}"
                        " not found"
                    ),
                )
            ...
            if payload.guest_count > room_type.max_occupancy:
                raise HTTPException(
                    status_code=status.HTTP_400_BAD_REQUEST,
                    detail=(
                        f"Guest count ({payload.guest_count})"
                        f" exceeds max occupancy"
                        f" ({room_type.max_occupancy})"
                    ),
                )
            ...
            available = room_type.total_rooms - max_booked
            ...
            if available <= 0:
                raise HTTPException(
                    ...
                    detail=(
                        ...
                        " is fully booked for selected dates."
                    ),
                )
            ...
            code = f"ZEN-{uuid.uuid4().hex[:8].upper()}"
