"""Search and price-trend projection service."""
...
class SearchService:
    ...
    async def search_rooms(
        ...
        cache = get_cache()
        cache_key = (
            f"search:{city}:{check_in_date}"
            f":{check_out_date}:{guests}"
        ...
        prop_stmt = select(Property).options(
            selectinload(Property.room_types),
        )
        if city:
            prop_stmt = prop_stmt.where(
        ...
        for prop in properties:
            for room in prop.room_types:
                ...
                booked_counts = (
                    await BookingService.get_booked_count_per_date(
                        session,
                        room.id,
                        check_in_date,
                        check_out_date,
                ...
                max_booked = (
                    max(booked_counts.values())
                    if booked_counts
                    else 0
                )
                available_rooms = (
                    room.total_rooms - max_booked
                )

                if available_rooms <= 0:
                    continue

                total_price, breakdowns = (
                    await BookingService.calculate_stay_pricing(
                        session,
                        room,
                        check_in_date,
                        check_out_date,
                ...
                avg_nightly = round(
                    total_price / max(night_count, 1), 2,
                )

                if (
                    min_price is not None
                    and avg_nightly < min_price
                ):
                    continue
                if (
                    max_price is not None
                    and avg_nightly > max_price
                ):
                    continue

                # Value score formula
                score_raw = (
                    (prop.review_score * prop.star_rating)
                    / max(avg_nightly**0.5, 1.0)
                ) * 10
                value_score = round(score_raw, 2)

                amenities_list = [
                    a.strip()
                    for a in prop.amenities.split(",")
                    if a.strip()
                ]
        ...
        if sort_by == "price_low":
            results.sort(
                key=lambda x: x.average_nightly_price,
            )
    ...
    async def get_price_trend(
        ...
        if not room:
            raise HTTPException(
                ...
                detail=(
                    f"RoomType ID {room_type_id} not found"
                ),
            )
        ...
        while curr < end_date:
            ...
            date_event = next(
                (
                    e for e in events
                    if e.start_date <= curr <= e.end_date
                ),
                None,
            )
            e_mult = (
                date_event.multiplier
                if date_event else 1.0
            )
            e_name = (
                date_event.name if date_event else None
            )

            breakdown = (
                pricing_engine.calculate_nightly_price(
            ...
            points.append(
                PriceTrendPoint(
                    ...
                    base_price=room.base_price,
                    occupancy_rate=(
                        breakdown.occupancy_rate
                    ),
                    occupancy_multiplier=(
                        breakdown.occupancy_multiplier
                    ),
                    lead_time_multiplier=(
                        breakdown.lead_time_multiplier
                    ),
                    event_multiplier=(
                        breakdown.demand_event_multiplier
                    ),
                    event_name=(
                        breakdown.demand_event_name
                    ),
                )
            )
